...
function HomePage() {
    ...
    async function fetchData() {
        await axios
            .get("http://localhost:8000/item/allItems")
        ...
        <h1>Catalogue</h1>
            <div class="row row-cols-1 row-cols-md-3 g-4 pb-5 pl-4 pr-4 pt-4">
                {Array.isArray(itemList) ? itemList.map(ele => (
                    <ItemCard title={ele.Title} description={ele.Description} price={ele.Price} sellerID={ele.SellerID} image={ele.Image} check={false}/>
                )) : []}
            </div>
        </div>
